 ...
 */
type ExtrinsicTreeItemProps = Omit<TreeItemProps, 'itemType' | 'value' | 'parentValue'>;
...
export class TreeHelper<Props extends ExtrinsicTreeItemProps> {
  ...
  public addNode(
    ...
    const newNode = new TreeNode<Props>(props);
    ...
    if (!parentValue) {
      ...
      if (!parentNode) {
        console.error(`[Tree addNode] Parent node with value ${parentValue} not found.`);
        return;
      }
  ...
  public updateNode(value: TreeItemValue, nodeProps: Props, itemType?: TreeItemType): void {
  ...
  public removeNode(value: TreeItemValue): void {
    ...
    if (!nodeToRemove) {
      console.error(`[Tree removeNode] Node with value ${value} not found. Nothing will be removed.`);
      return;
    }
    ...
    if (!nodeToRemove.parentValue) {
      ...
      parentNode.subtree = parentNode.subtree.filter(node => node.value !== value);
    }
  }

  public moveNode(value: TreeItemValue, parentValue?: TreeItemValue, position?: number): void {
    ...
    const moveInSameSubtree = (subtree: TreeNode<Props>[], nodeToMove: TreeNode<Props>, position?: number) => {
      const filteredSubtree = subtree.filter(node => node.value !== nodeToMove.value);
      if (position !== undefined) {
    ...
    } else if ((!parentValue && nodeToMove.parentValue) || (parentValue && nodeToMove.parentValue === parentValue)) {
      const parentNode = this.nodes.get(nodeToMove.parentValue);
      if (!parentNode) {
        console.error(`[Tree moveNode] Parent node with value ${parentValue} not found.`);
        return;
      }
      parentNode.subtree = moveInSameSubtree(parentNode.subtree, nodeToMove, position);
    } else {
  ...
  public getTree(value?: TreeItemValue): TreeNode<Props> | null | TreeNode<Props>[] {
    if (value) {
  ...
  public getTreeForRender(value?: TreeItemValue): TreeNodeWithSubtree<Props> | null | TreeNodeWithSubtree<Props>[] {
  ...
  public getFlatTreeForRender(value?: TreeItemValue): FlatTreeNodeForRender<Props>[] {
    ...
    if (Array.isArray(tree)) {
      ...
      if (value) {
        const node = this.nodes.get(value);
        const parentNode = node?.parentValue !== undefined ? this.nodes.get(node?.parentValue) : undefined;
        if (parentNode) {
          position = parentNode.subtree.findIndex(child => child.value === value);
        }
      }
  ...
  private createTreeItemForRender(node: TreeNode<Props>): TreeNodeWithSubtree<Props> {
    return {
      ...node._nodeProps,
      value: node.value,
  ...
  private flattenTree(node: TreeNodeWithSubtree<Props>, position: number): FlatTreeNodeForRender<Props>[] {
    const flattenedNode: FlatTreeNodeForRender<Props> = {
      ...node,
      position,
      subtree: [],
    };
    const flattenedSubtree = node.subtree.flatMap((child, index) => this.flattenTree(child, index));
