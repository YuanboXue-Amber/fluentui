...
export interface VirtualStickyTreePaginationProps extends VirtualStickyTreeProps {
  ...
  loadingPlaceHolderItem?: ShorthandValue<TreeItemProps>;
}
...
export const VirtualStickyTreePagination: ComponentWithAs<'div', VirtualStickyTreePaginationProps> = (props) => {
  ...
  const { items, children, className, design, styles, variables, height, stickyItemSize } = props;
  ...
  const { hasNextPage, loadingPlaceHolderItem } = props;
  const itemWithLoading = React.useMemo<ObjectShorthandCollection<TreeItemProps>>(() => {
    if (!hasNextPage) {
      return items;
    }
    return [...items, loadingPlaceHolderItem] as ObjectShorthandCollection<TreeItemProps>;
  }, [hasNextPage, items, loadingPlaceHolderItem]);
  ...
  } = useVirtualStickyTree({ ...props, items: itemWithLoading });
  ...
  const { isNextPageLoading, onLoadNextPage, paginationThreshold } = props;
  ...
  const itemCount = visibleItemIds.length;
  ...
  const loadMoreItems = React.useCallback(() => {
    ...
    return onLoadNextPage().then(() => {
      if (listRef.current) {
        // this fix means we don't have to set the loading size in consumer using the component
        ...
        >
          <InnerElementContext.Provider value={innerElementContextValue}>
            <InfiniteLoader
              ...
              threshold={paginationThreshold} // pre-fetch whenScrolled within x amount of rows
            >
              {({ onItemsRendered, ref }) => (
                <VariableSizeList
...
VirtualStickyTreePagination.defaultProps = {
  ...
  loadingPlaceHolderItem: { id: 'loading', title: 'loading...' },
};
